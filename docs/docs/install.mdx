...
# Specify the version of Infrahub to run (for example, 1.5.0b2, stable, latest)
# Default: 1.5.0b2
...
Configure ContainerLab for virtual network device simulation:

```bash
# Docker image for Arista cEOS (Container EOS)
# Used for network device simulation in containerlab topologies
# Required if you plan to use containerlab for virtual lab deployment
