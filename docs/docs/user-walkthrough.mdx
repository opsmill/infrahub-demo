...
- **Cisco NX-OS** - The default DC-2 design (`objects/dc-cisco-s.yml`)
...
```bash
# Juniper design (DC-5)
uv run infrahubctl object load objects/dc-juniper-s.yml --branch add-dc5

# SONiC design (DC-4)
uv run infrahubctl object load objects/dc-sonic-border-leafs.yml --branch add-dc4
```
