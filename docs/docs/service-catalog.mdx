...
### Option 2: using Docker Compose profiles

Alternatively, you can start Infrahub with the Service Catalog profile:
...
You should see a container named `infrahub-demo-service-catalog-1` or similar in the output.
...
:::info
If the page doesn't load, verify the container is running and check the logs with `docker logs infrahub-demo-service-catalog-1`.
:::
...
```bash
docker logs infrahub-demo-service-catalog-1
```
...
```bash
docker inspect -f '{{range .NetworkSettings.Networks}}{{.IPAddress}}{{end}}' infrahub-demo-service-catalog-1
```
...
```bash
docker exec infrahub-demo-service-catalog-1 curl http://infrahub-server:8000/api/schema
```
...
```bash
docker logs infrahub-demo-service-catalog-1 | grep -i error
```
...
```bash
docker logs -f infrahub-demo-service-catalog-1
```
